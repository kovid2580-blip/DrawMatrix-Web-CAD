...
import { useThreeStore, Layer } from "@/store";
import { Eye, EyeOff, Lock, Unlock, Plus, Trash2 } from "lucide-react";

export const LayerManager = () => {
    const { layers, activeLayerId, setActiveLayer, updateLayer, addLayer } = useThreeStore();
    ...
    return (
        <div className="flex flex-col h-full bg-[#1e1f22] text-xs">
            <div className="p-3 border-b border-black flex items-center justify-between bg-black/20">
                <span className="font-bold uppercase tracking-widest text-emerald-500">Layers</span>
                <button onClick={handleAddLayer} className="p-1 hover:bg-white/10 rounded transition-colors">
                    <Plus size={14} />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto py-1">
                {layers.map((layer) => (
                    ...
                    >
                        ...
                        <div className="w-3 h-3 rounded-full border border-white/10" style={{ backgroundColor: layer.color }} />

                        <span className={`flex-1 truncate ${activeLayerId === layer.id ? "text-white font-bold" : "text-gray-400"}`}>
                            {layer.name}
                        </span>
                    </div>
                ))}
