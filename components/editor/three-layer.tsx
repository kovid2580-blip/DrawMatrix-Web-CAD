...
import { useThreeStore, ThreeObject, Layer } from "@/store";
...
const USER_ID = typeof window !== 'undefined' ? (window.localStorage.getItem('cad_user_id') || Math.random().toString(36).substr(2, 9)) : 'guest';
if (typeof window !== 'undefined') window.localStorage.setItem('cad_user_id', USER_ID);
...
const DraftingObject = ({ obj, layer, isSelected, onClick }: any) => {
    const points = useMemo(() => {
        switch (obj.type) {
            ...
            case "circle":
                const curve = new THREE.EllipseCurve(0, 0, obj.properties.radius || 1, obj.properties.radius || 1, 0, 2 * Math.PI, false, 0);
            ...
            case "arc":
                const arcCurve = new THREE.ArcCurve(0, 0, obj.properties.radius || 1, 0, Math.PI, false);
                return arcCurve.getPoints(32).map(p => new THREE.Vector3(p.x, 0, p.y));
            default:
                return [];
        }
    }, [obj]);
 ...
 */
const CADObject = ({ obj, objects, isSelected, onSelect, setRef, activeTool }: any) => {
    ...
    const material = (
        <meshStandardMaterial
            color={isSelected ? "#ffcc00" : isLocked ? "#444" : (layer?.color || obj.color)}
            opacity={isLocked ? 0.5 : 1}
            transparent={isLocked}
            wireframe={isLocked && !isSelected}
            clippingPlanes={obj.properties.clippingEnabled ? [new THREE.Plane(new THREE.Vector3(0, -1, 0), 0.5)] : []}
            clipShadows={true}
        />
    );
    ...
    if (["line", "circle", "arc", "rect", "ellipse"].includes(obj.type)) {
        return <DraftingObject obj={obj} layer={layer} isSelected={isSelected} onClick={handleClick} />;
    }
    ...
    const geometry = useMemo(() => {
        switch (obj.type) {
            case "wall":
                return GeometryEngine.createWall(obj.properties.width || 4, obj.properties.height || 2.5, obj.properties.thickness || 0.2);
            case "slab":
                return GeometryEngine.createSlab(obj.properties.width || 4, obj.properties.depth || 4, obj.properties.thickness || 0.2);
            case "stairs":
                return GeometryEngine.createStair(obj.properties.stepCount || 10, 0.18, 0.28, 1.2);
            case "roof":
                return GeometryEngine.createRoof(obj.properties.width || 5, obj.properties.depth || 5, obj.properties.pitch || 30);
            case "dimension":
                const start = new THREE.Vector3(...(obj.properties.start || [0, 0, 0]));
                const end = new THREE.Vector3(...(obj.properties.end || [2, 0, 0]));
                return GeometryEngine.createDimensionLines(start, end, obj.properties.offset || 0.5);
            default:
                return null;
        }
    ...
    if (obj.type === "dimension") {
        const dist = new THREE.Vector3(...(obj.properties.start || [0, 0, 0])).distanceTo(new THREE.Vector3(...(obj.properties.end || [2, 0, 0]))).toFixed(2);
        return (
            <group position={obj.transform.position}>
                <primitive object={geometry as object} attach="geometry" />
                <meshBasicMaterial color={isSelected ? "#ffcc00" : (layer?.color || "#ffffff")} />
                <Html position={[1, 0.6, 0]} center>
                    <div className="text-[10px] text-cyan-400 font-bold bg-black/80 px-1 rounded">{dist}m</div>
                </Html>
            </group>
        );
    }
    ...
    if (geometry) {
        return (
            <mesh {...commonProps}>
                <primitive object={(obj.geometryData ? new THREE.BufferGeometry().copy(JSON.parse(obj.geometryData)) : geometry) as object} attach="geometry" />
                {material}
            </mesh>
        );
    }
...
const ThreeScene = ({ transformMode, activeTool, setTool }: { transformMode: "translate" | "rotate" | "scale", activeTool: string, setTool: (tool: string) => void }) => {
    const {
        objects,
        selectedObjectId,
        setSelectedObjectId,
    ...
    useFrame((state) => {
        ...
        if (lightRef.current) {
            lightRef.current.position.set(camera.position.x + 50, 100, camera.position.z + 50);
    ...
    const snap = useCallback((pt: THREE.Vector3) => {
    ...
    useEffect(() => {
        ...
        socket.on("transform_object", ({ objectId, payload }) => updateObject(objectId, { transform: payload.transform }));
        socket.on("update_property", ({ objectId, payload }) => updateObject(objectId, { properties: payload.properties }));

        socket.on("presence-update", (data) => updatePresence(data.userId, data.presence));
        socket.on("presence-disconnect", (userId) => removePresence(userId));
        socket.on("replace_geometry", ({ objectId, geometryData }) => updateObject(objectId, { geometryData }));

        return () => { socket.disconnect(); };
    }, [addObject, deleteObject, updateObject, updatePresence, removePresence]);

    // Cursor tracking
    const handleTransformChange = useCallback((isFinal = false) => {
        if (selectedMesh && selectedObjectId) {
            ...
            if (isFinal) {
                ...
                if (movingObj && (movingObj.type === "door" || movingObj.type === "window")) {
                    const nearestWall = objects.find(o => o.type === "wall" &&
                        new THREE.Vector3(...o.transform.position).distanceTo(pos) < 2);

                    if (nearestWall) {
                        const wallGeom = GeometryEngine.createWall(nearestWall.properties.width || 4, nearestWall.properties.height || 2.5, nearestWall.properties.thickness || 0.2);
                        const openingGeom = movingObj.type === "door" ? new THREE.BoxGeometry(0.9, 2.1, 0.4) : new THREE.BoxGeometry(1.2, 1.2, 0.4);

                        const resultGeom = GeometryEngine.cutOpeningInWall(
                            wallGeom, nearestWall.transform.position,
                            openingGeom, [pos.x, pos.y, pos.z]
                        );
    ...
    const handlePointerDown = useCallback((e: any) => {
        if (activeTool === "select" || (activeTool as any) === "none" || cinematicMode) return;
    ...
    const handlePointerUp = useCallback((e: any) => {
        ...
        const props: any = {};
        let pos: [number, number, number] = [drawingStart.x, drawingStart.y, drawingStart.z];

        if (activeTool === "line") {
            props.start = [0, 0, 0];
            props.end = [currentPoint.x - drawingStart.x, 0, currentPoint.z - drawingStart.z];
        } else if (activeTool === "circle") {
            props.radius = drawingStart.distanceTo(currentPoint);
        } else if (activeTool === "box" || activeTool === "wall" || activeTool === "slab" || activeTool === "rect") {
            props.width = Math.abs(currentPoint.x - drawingStart.x) || 1;
            props.depth = Math.abs(currentPoint.z - drawingStart.z) || 1;
            props.height = 2.5;
            props.thickness = 0.2;
            pos = [(drawingStart.x + currentPoint.x) / 2, 0, (drawingStart.z + currentPoint.z) / 2];
        }
        ...
        addObject(newObj);
        socket.emit("create_object", { type: 'create_object', objectId: newObj.id, userId, timestamp: Date.now(), payload: newObj });

        // Architectural Logic for New Placement: Auto-cut openings
        if (newObj.type === "door" || newObj.type === "window") {
            const nearestWall = objects.find(o => o.type === "wall" &&
                new THREE.Vector3(...o.transform.position).distanceTo(new THREE.Vector3(...pos)) < 2);

            if (nearestWall) {
                const wallGeom = GeometryEngine.createWall(nearestWall.properties.width || 4, nearestWall.properties.height || 2.5, nearestWall.properties.thickness || 0.2);
                const openingGeom = newObj.type === "door" ? new THREE.BoxGeometry(0.9, 2.1, 0.4) : new THREE.BoxGeometry(1.2, 1.2, 0.4);
                ...
                const geometryData = JSON.stringify(resultGeom.toJSON());
                updateObject(nearestWall.id, { geometryData });
                socket.emit("replace_geometry", { objectId: nearestWall.id, geometryData, userId });
            }
        }
        ...
        setTool("select");
    }, [drawingStart, currentPoint, activeTool, activeLayerId, addObject, updateObject, objects, setTool, cinematicMode]);
        ...
        <group>
            ...
            <Suspense fallback={null}>
                ...
                <OrbitControls
                    ...
                    enableRotate={(activeTool === "select" || (activeTool as any) === "none") && viewMode === "3D" && !selectedMesh}
...
export default function ThreeLayer({ activeTool, setTool }: { activeTool: string, setTool: (tool: string) => void }) {
    const { selectedObjectId, setSelectedObjectId, deleteObject, presences, viewMode, cinematicMode, setCinematicMode } = useThreeStore();
    const [transformMode, setTransformMode] = useState<"translate" | "rotate" | "scale">("translate");

    return (
        <div className="flex-1 bg-[#121214] relative overflow-hidden h-full">
            <Canvas shadows gl={{ antialias: true, localClippingEnabled: true }} onPointerMissed={() => setSelectedObjectId(null)}>
                {viewMode === "3D" ? (
                    <PerspectiveCamera makeDefault position={[12, 12, 12]} fov={35} far={100000} />
                ) : (
                    <OrthographicCamera makeDefault
                        position={viewMode === "2D" ? [0, 20, 0] : [20, 0, 0]}
                        zoom={40}
                        rotation={viewMode === "2D" ? [-Math.PI / 2, 0, 0] : [0, Math.PI / 2, 0]}
                        far={100000}
                    />
                )}
                <ThreeScene transformMode={transformMode} activeTool={activeTool} setTool={setTool} />
            </Canvas>

            {/* CAD HUD */}
            <div className={`absolute top-4 left-4 flex flex-col space-y-3 pointer-events-none text-white font-mono transition-opacity duration-700 ${cinematicMode ? "opacity-0 invisible" : "opacity-100 visible"}`}>
                <div className="bg-black/80 backdrop-blur-xl px-4 py-2 rounded-lg border border-white/10 flex items-center gap-4 shadow-2xl">
                    <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full animate-pulse ${socket.connected ? "bg-cyan-500" : "bg-rose-500"}`} />
                        <span className="text-[10px] uppercase tracking-[0.2em] font-black italic">Matrix::Core</span>
                    </div>
                        ...
                        <div className="flex space-x-1 pt-2">
                            <button
                                onClick={() => {
                                    ...
                                    socket.emit("delete_object", { objectId: selectedObjectId, userId: USER_ID });
                                }}
