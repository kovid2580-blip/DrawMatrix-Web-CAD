...
export const FloatingVideoCall = () => {
    const { inCall, channelName, isMinimized, leaveCall, toggleMinimize } = useCall();
            ...
            <AnimatePresence>
                {inCall && (
                    <motion.div
                        ...
                        className={`fixed z-[100] bg-slate-900 border border-slate-700 shadow-2xl overflow-hidden flex flex-col ${isMinimized ? "bottom-4 right-4" : "top-8 left-[9vw]"
                            }`}
                    ...
                    >
                        ...
                        <div className="h-10 bg-slate-800 flex items-center justify-between px-4 cursor-move select-none shrink-0 border-b border-slate-700">
                            ...
                            <div className="flex items-center gap-3">
                                <button onClick={toggleMinimize} title={isMinimized ? "Expand" : "Minimise"}
                                    className="text-slate-400 hover:text-white transition-colors">
                                    {isMinimized ? <Maximize2 size={14} /> : <Minimize2 size={14} />}
                                </button>
                                <button onClick={leaveCall} title="End call"
