...
export class GeometryEngine {
  ...
  static extrude(
    ...
    const shape = new THREE.Shape(
      shapePoints.map((p) => new THREE.Vector2(p[0], p[1]))
    );
  ...
  static sweep(
    ...
    const profile = new THREE.Shape(
      profilePoints.map((p) => new THREE.Vector2(p[0], p[1]))
    );
  ...
  static cutOpeningInWall(
    wallGeometry: THREE.BufferGeometry,
    wallPos: [number, number, number],
    openingGeometry: THREE.BufferGeometry,
    openingPos: [number, number, number]
  ): THREE.BufferGeometry {
