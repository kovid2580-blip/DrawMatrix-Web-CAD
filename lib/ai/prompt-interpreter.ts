...
import { ParsedEntity, ParsedPrompt } from "@/store/ai-store";
...
function findBestMatch(prompt: string): Partial<ParsedEntity>[] | null {
  ...
  // 1. Exact Match
  const exact = ALL_TRAINING_SAMPLES.find(
    (s) => s.prompt.toLowerCase() === normalized
  );
  if (exact) return exact.expected;

  // 2. Fuzzy Match (Simple inclusion for now, can be upgraded to Levenshtein)
  const fuzzy = ALL_TRAINING_SAMPLES.find(
    (s) =>
      normalized.includes(s.prompt.toLowerCase()) ||
...
const RADIUS_PATTERN =
  /(?:radius|r)\s*(?:of\s+)?(\d+(?:\.\d+)?)\s*(?:m|meters?)?/i;
const HEIGHT_PATTERN =
  /(?:height|tall|high)\s*(?:of\s+)?(\d+(?:\.\d+)?)\s*(?:m|meters?)?/i;
const WIDTH_PATTERN_SINGLE =
  /(?:width|wide)\s*(?:of\s+)?(\d+(?:\.\d+)?)\s*(?:m|meters?)?/i;
const DEPTH_PATTERN =
  /(?:depth|deep|long)\s*(?:of\s+)?(\d+(?:\.\d+)?)\s*(?:m|meters?)?/i;
const THICKNESS_PATTERN =
  /(?:thickness|thick)\s*(?:of\s+)?(\d+(?:\.\d+)?)\s*(?:m|meters?)?/i;
...
function mapDimensionsForEntity(
  raw: RawDimensions,
  entityType: ParsedEntity["type"]
): ParsedEntity["dimensions"] {
...
function extractElementCount(text: string, element: string): number {
  const patterns = [
    ...
    new RegExp(
      `(one|an?|two|three|four|five|six|seven|eight|nine|ten|twelve|fifteen|twenty)\\s*${element}`,
      "i"
    ),
  ];
...
export function interpretPrompt(prompt: string): ParsedPrompt {
  ...
  if (normalized.includes("office") || normalized.includes("workspace"))
    buildingType = "office";
  else if (normalized.includes("hospital") || normalized.includes("clinic"))
    buildingType = "hospital";
  else if (normalized.includes("school") || normalized.includes("classroom"))
    buildingType = "school";
  else if (
  ...
  if (floorMatch) floors = parseInt(floorMatch[1]);
    ...
    normalized.includes("two floor") ||
    normalized.includes("double story")
  )
    floors = 2;
  else if (
    normalized.includes("three floor") ||
    normalized.includes("triple story")
  )
    floors = 3;
  ...
  for (const [type, patterns] of Object.entries(ENTITY_KEYWORDS) as [
    ParsedEntity["type"],
    RegExp[],
  ][]) {
  ...
  const materialMatch = normalized.match(
    /use\s+(\w+)\s+(?:walls|materials|slabs)/i
  );
  ...
  for (const type of ROOM_TYPES) {
    const count = extractElementCount(
      normalized,
      type.replace("_", " ") + "s?"
    );
    if (count > 0) {
  ...
  detectedTypes.forEach((type) => {
    ...
    if (["column", "window", "door"].includes(type)) {
      ...
      type === "building" &&
      (normalized.includes("house") || normalized.includes("block"))
    ) {
