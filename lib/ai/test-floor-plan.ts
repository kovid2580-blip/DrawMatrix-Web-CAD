...
async function testPipeline() {
    ...
    const prompt = "Generate a small house with two bedrooms, a living room, kitchen and bathroom";
    ...
    console.log(`- Rooms Detected: ${parsed.entities.filter(e => e.label !== "corridor").length}`);
    ...
    if (living && kitchen) {
        ...
        console.log(`- Adjacency (Living/Kitchen): ${adjacencyCorrect ? "✅ Correct" : "❌ Failed"}`);
    }
    console.log(`- Corridor (Step 8): ${corridor ? "✅ Generated" : "❌ Missing"}`);
    ...
    console.log("\nDoor Widths found:");
    doors.forEach(d => console.log(`  - ${d.params.width}m at ${d.params.x},${d.params.z}`));

    const entranceDoor = doors.find(d => d.params.width === 1.2);
    const roomDoor = doors.find(d => d.params.width === 0.9);
    console.log(`- Entrance Door (1.2m) (Step 6): ${entranceDoor ? "✅ Found" : "❌ Missing"}`);
    console.log(`- Room Doors (0.9m) (Step 6): ${roomDoor ? "✅ Found" : "❌ Missing"}`);

    const windowSizes = windows.every(w => w.params.width === 1.5);
    console.log(`- Standard Windows (1.5m) (Step 7): ${windowSizes ? "✅ Correct" : "❌ Invalid"}`);
