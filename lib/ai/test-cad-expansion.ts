...
function verifyCADExpansion() {
  ...
  for (const prompt of tests) {
    ...
    console.log(
      `Entities detected: ${interpreted.entities.map((e) => e.type).join(", ")}`
    );
    ...
    if (prompt.includes("hospital")) {
      ...
      console.log(
        hasER && hasICU
          ? "✅ Hospital Template Matched"
          : "❌ Hospital Template Failed"
      );
    }

    if (prompt.includes("residential block")) {
      const houseCount = commands.filter((c) =>
        c.params.label?.startsWith("House")
      ).length;
      console.log(
        houseCount >= 5
          ? `✅ Urban Block Matched (${houseCount} houses)`
          : `❌ Urban Block Failed (${houseCount} houses)`
      );
    }

    if (prompt.includes("parking")) {
      const hasSlab = commands.some(
        (c) => c.action === "create_slab" && c.params.label === "parking_lot"
      );
    ...
    if (prompt.includes("steel walls")) {
      ...
      console.log(
        hasMaterial
          ? "✅ Material Instruction Matched (Steel)"
          : "❌ Material Instruction Failed"
      );
    }
  }
}
