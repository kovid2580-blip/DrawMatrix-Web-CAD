import { ParsedPrompt, ParsedEntity, DrawCommand } from "@/store/ai-store";
import { ARCHITECTURAL_DEFAULTS } from "./requirement-engine";
import { detectLayoutType, generateArchitecturalSpecs, RoomSpec, specsToCommands } from "./layout-generator";

// ── Fallback Individual Entity Dispatcher ────────────────────────────────────

function entityToCommands(entity: ParsedEntity, context: { offsetX: number; offsetZ: number }): DrawCommand[] {
    const label = entity.label?.toLowerCase() || "";
    const type = entity.type;

    // Handle generic room types and specific room types
    const isRoomType = [
        "room", "rect", "bedroom", "master_bedroom", "living_room",
        "kitchen", "bathroom", "conference_room", "office_layout",
        "office", "meeting_room", "classroom", "ward", "icu"
    ].includes(type);
...
export function parseDesignIntent(parsed: ParsedPrompt): DrawCommand[] {
    ...
    if (layoutType) {
        const houseCount = parsed.entities.find(e => e.type === "building")?.dimensions.count;
