        ...
        switch (c.action) {
            case "create_wall": {
                if (c.params.thickness < RULES.wall.minThickness) {
                    warnings.push(`Wall thickness ${c.params.thickness}m is below minimum ${RULES.wall.minThickness}m – auto-corrected.`);
                    c.params.thickness = RULES.wall.interiorThickness;
                }
                if (c.params.thickness > RULES.wall.maxThickness) {
                    warnings.push(`Wall thickness ${c.params.thickness}m exceeds maximum – clamped to ${RULES.wall.maxThickness}m.`);
                    c.params.thickness = RULES.wall.maxThickness;
                }
                if (c.params.height > RULES.wall.maxHeight) {
                    warnings.push(`Wall height ${c.params.height}m exceeds ${RULES.wall.maxHeight}m – clamped.`);
            ...
            case "insert_door": {
                if (c.params.width < RULES.door.minWidth) {
                    warnings.push(`Door width ${c.params.width}m is below minimum ${RULES.door.minWidth}m – corrected to ${RULES.door.standardWidth}m.`);
                    c.params.width = RULES.door.standardWidth;
                }
                if (c.params.width > RULES.door.maxWidth) {
                    warnings.push(`Door width ${c.params.width}m exceeds maximum – clamped to ${RULES.door.maxWidth}m.`);
            ...
            case "insert_window": {
                if (c.params.width < RULES.window.minWidth) {
                    warnings.push(`Window width ${c.params.width}m is below minimum – corrected to ${RULES.window.standardWidth}m.`);
                    c.params.width = RULES.window.standardWidth;
                }
                if (c.params.width > RULES.window.maxWidth) {
                    warnings.push(`Window width ${c.params.width}m exceeds maximum – clamped.`);
            ...
            case "create_stairs": {
                if (c.params.riserHeight > RULES.stairs.maxRiserHeight) {
                    warnings.push(`Stair riser height ${c.params.riserHeight}m exceeds max ${RULES.stairs.maxRiserHeight}m – corrected.`);
                    c.params.riserHeight = RULES.stairs.maxRiserHeight;
                }
                if (c.params.treadDepth < RULES.stairs.minTreadDepth) {
                    warnings.push(`Stair tread depth ${c.params.treadDepth}m is below minimum ${RULES.stairs.minTreadDepth}m – corrected.`);
                    c.params.treadDepth = RULES.stairs.minTreadDepth;
                }
                if (c.params.width < RULES.stairs.minWidth) {
                    warnings.push(`Stair width ${c.params.width}m is below minimum – corrected to ${RULES.stairs.standardWidth}m.`);
