...
import { GOLDEN_SAMPLES, ALL_TRAINING_SAMPLES } from "./datasets/training-samples";
...
function testAI() {
    ...
    for (const sample of GOLDEN_SAMPLES) {
        ...
        const match = result.entities.length === sample.expected.length &&
    ...
    const subset = ALL_TRAINING_SAMPLES.sort(() => 0.5 - Math.random()).slice(0, 50);
    for (const sample of subset) {
        const result = interpretPrompt(sample.prompt);
        const match = result.entities.length >= sample.expected.length &&
            sample.expected.every(exp => result.entities.some(act => act.type === exp.type));
