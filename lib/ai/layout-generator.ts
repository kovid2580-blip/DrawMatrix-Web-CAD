...
import { ARCHITECTURAL_DEFAULTS, getRoomRequirements } from "./requirement-engine";
...
function addCorridor(rooms: RoomSpec[]): RoomSpec[] {
    ...
    const bedroomWidth = rooms.filter(r => r.type === "bedroom").reduce((sum, r) => sum + r.width, 0);
...
function generate1BHK(): RoomSpec[] {
    const reqs = getRoomRequirements(["living_room", "bedroom", "kitchen", "bathroom"]);
    return reqs.map(r => ({
        ...r,
...
function generate2BHK(): RoomSpec[] {
    const reqs = getRoomRequirements(["living_room", "bedroom", "bedroom", "kitchen", "bathroom"]);
    const rooms = reqs.map((r, i) => ({
        ...r,
        name: r.type === "bedroom" ? `BEDROOM ${i + 1}` : r.type.replace("_", " ").toUpperCase(),
        hasDoor: true,
        hasWindow: r.type !== "bathroom",
...
function generate3BHK(): RoomSpec[] {
    const reqs = getRoomRequirements(["living_room", "bedroom", "bedroom", "bedroom", "kitchen", "bathroom", "bathroom"]);
    let bedIdx = 1;
    let bathIdx = 1;
...
function generateOffice(): RoomSpec[] {
    const reqs = getRoomRequirements(["office", "office", "meeting_room", "kitchen", "bathroom"]);
    return reqs.map((r, i) => ({
        ...r,
        name: r.type === "office" ? `OFFICE ${i + 1}` : r.type.replace("_", " ").toUpperCase(),
        hasDoor: true,
        hasWindow: true,
        windowCount: 2
    }));
...
function generateHospital(): RoomSpec[] {
    const reqs = getRoomRequirements(["living_room", "bedroom", "bedroom", "bedroom", "bathroom"]); // Mapping to hospital types via logic later or direct
    return reqs.map((r, i) => ({
        ...r,
...
function generateUrbanBlock(houseCount = 10): RoomSpec[] {
    ...
    for (let i = 1; i <= (houseCount || 10); i++) {
        houses.push({
            ...
            type: "house",
            width: 8, depth: 10,
            hasDoor: true, hasWindow: true, windowCount: 4
        });
    }
    return houses;
}
...
function layoutRooms(rooms: RoomSpec[]): RoomSpec[] {
    ...
    const frontZone = rooms.filter(r => ["living_room", "kitchen", "reception", "entrance", "dining_room"].includes(r.type));
    const backZone = rooms.filter(r => ["bedroom", "bathroom", "office", "ward", "icu", "meeting_room"].includes(r.type));
    const corridor = rooms.find(r => r.type === "corridor");
    const others = rooms.filter(r => !frontZone.includes(r) && !backZone.includes(r) && r !== corridor);

    // 2. Standardize Depths per row to ensure perfect adjacency
    const frontDepth = frontZone.length > 0 ? Math.max(...frontZone.map(r => r.depth)) : 0;
    const backDepth = backZone.length > 0 ? Math.max(...backZone.map(r => r.depth)) : 0;
...
export function specsToCommands(specs: RoomSpec[]): DrawCommand[] {
    ...
    for (const spec of specs) {
        ...
        const walls = [
            ...
            { name: "east", start: [x + w, 0, z + wallT], end: [x + w, 0, z + d - wallT] },
        ];
        ...
        if (spec.hasDoor) {
            const isEntrance = label.toUpperCase().includes("LIVING") || label.toUpperCase().includes("RECEPTION");
            commands.push({
                action: "insert_door",
                params: {
                    x: x + w / 2,
                    z: z,
                    width: isEntrance ? ARCHITECTURAL_DEFAULTS.entrance_door_width : ARCHITECTURAL_DEFAULTS.room_door_width,
                    height: 2.1,
                    wall: "south",
                },
            });
...
export function detectLayoutType(prompt: string): string | null {
    ...
    if (/\b(?:1\s*b(?:ed)?(?:room)?|1\s*bhk|studio)\b/i.test(lower)) return "1bhk";
    if (/\b(?:2\s*b(?:ed)?(?:room)?|2\s*bhk|apartment|flat)\b/i.test(lower)) return "2bhk";
    if (/\b(?:3\s*b(?:ed)?(?:room)?|3\s*bhk)\b/i.test(lower)) return "3bhk";
    if (/\b(?:office|workspace|conference|meeting)\b/i.test(lower)) return "office";
...
export function generateArchitecturalSpecs(layoutType: string, count?: number): RoomSpec[] {
    switch (layoutType) {
        case "1bhk": return layoutRooms(generate1BHK());
        case "2bhk": return layoutRooms(generate2BHK());
        case "3bhk": return layoutRooms(generate3BHK());
        case "office": return layoutRooms(generateOffice());
        case "hospital": return layoutRooms(generateHospital());
        case "urban_block": return layoutRooms(generateUrbanBlock(count));
        default: return layoutRooms(generate2BHK());
    }
}
