...
export type ThreeObjectType =
    | "line" | "polyline" | "circle" | "arc" | "rect" | "polygon" | "ellipse" | "spline" | "hatch" // 2D
    | "box" | "cylinder" | "sphere" // 3D Primitives
    | "wall" | "door" | "window" | "slab" | "stairs" | "roof" // Architectural
    | "comment" | "text" | "dimension"; // Annotation
...
interface ThreeStore {
    ...
    updateObject: (id: string, updates: Partial<ThreeObject>) => void;
...
export const useThreeStore = create<ThreeStore>((set, get) => ({
    layers: [
        { id: DEFAULT_LAYER_ID, name: "Default", visible: true, locked: false, color: "#ffffff", order: 0 }
    ],
    objects: [],
    ...
    updateObject: (id, updates) => {
        const state = get();
        if (state.userRole === "Viewer") return;
        // Optional: Only push history on "final" moves or certain types of updates
        // For now, let's keep it simple. If we want to optimize, we can pass a 'saveHistory' flag.
        set((state) => ({
    ...
    updatePresence: (userId, presence) =>
        set((state) => ({
            presences: {
                ...state.presences,
                [userId]: { ...state.presences[userId], ...presence, id: userId } as UserPresence
            }
        })),
    ...
    defineBlock: (name: string, objectIds: string[]) => set((state) => {
        if (state.userRole === "Viewer") return state;
        const blockObjects = state.objects.filter(o => objectIds.includes(o.id));
        const newBlock: BlockDefinition = {
            id: Math.random().toString(36).substr(2, 9),
            name,
            objects: JSON.parse(JSON.stringify(blockObjects))
        };
        return { blockLibrary: { ...state.blockLibrary, [newBlock.id]: newBlock } };
    }),

    insertBlock: (blockId: string, position: [number, number, number]) => set((state) => {
        if (state.userRole === "Viewer") return state;
